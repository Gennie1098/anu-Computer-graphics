...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (fabs(det) < EPSILON) // Check if the determinant is near zero
    {
    ...
    if (u < 0.0f || u > 1.0f) // Check if u is outside the triangle
    {
        return inter;
    }    

    Vector3f Q = crossProduct(T, e1);
    float v = dotProduct(ray.direction, Q) * invDet;
    if (v < 0.0f || u + v > 1.0f) // Check if v is outside the triangle
    {
         return inter;
    }   
    ...
    if (t > EPSILON) { // t must be larger than 0 to be in front of the ray's origin
        inter.happened = true;
        ...
        inter.tcoords =  Vector2f(u, v); // Setting the texture coordinate
